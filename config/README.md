...
只有「安安大冒險」出現玩家可見的新玩法、平衡調整或重要修正時，才更新 `config/update.toml` 的 `version` 與內容；AI、語音或維運變更不修改此檔。Bot 上線後會在每個伺服器尋找或建立「安安大冒險更新」唯讀頻道，並發布 Embed 公告；每個伺服器的發布紀錄保存在 `data/updates.db`，因此同版本重啟不會重複發送。Bot 需要「管理頻道」、「傳送訊息」與「嵌入連結」權限；單一伺服器發布失敗只會寫入 log，不會中斷其他功能。
