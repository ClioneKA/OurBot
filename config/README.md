...
- `AI_ROLE_IDS` 可設定逗號分隔的身分組 ID；設定後，伺服器成員至少需擁有其中一個才能觸發 AI。留空不限制，私訊不受影響。
...
| 檔案／區塊 | 內容 |
| --- | --- |
| `.env` | Discord Token、OpenAI／MiniMax API Key、FFmpeg／資料庫路徑、伺服器／頻道／身分組 ID、限頻豁免使用者 ID |
| `settings.toml` 的 `[ai]` | 一般模型、回覆機率、聊天歷史與輸入輸出長度 |
| `update.toml` | 安安大冒險部署後自動發布的版本、標題、摘要與更新項目 |
...
只有「安安大冒險」出現玩家可見的新玩法、平衡調整或重要修正時，才更新 `config/update.toml` 的 `version` 與內容；AI、語音或維運變更不修改此檔。Bot 上線後會在已登記的「安安大冒險」分類中尋找或建立「更新資訊」唯讀頻道，並發布 Embed 公告；尚未登記時需先執行 `/冒險區域 建立／匯入`。每個伺服器的發布紀錄保存在 `data/updates.db`，因此同版本重啟不會重複發送。Bot 需要「管理頻道」、「傳送訊息」與「嵌入連結」權限；單一伺服器發布失敗只會寫入 log，不會中斷其他功能。

RPG 頻道可先用 `.env` 配置，再由伺服器管理員執行 `/冒險區域 建立／匯入` 收編到資料庫；`/冒險區域 修復名稱、分類與權限` 會統一受管頻道名稱、分類與權限。酒館只供餐點公告，另建的冒險者聊天頻道開放成員發言。安安需要「管理頻道」與「管理身分組」。
