...
class TailorViewTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_equipment_first_socket_and_replace(self):
        ...
        crystal_select = next(child for child in view.children
                              if getattr(child, 'action', None) == 'crystal')
        self.assertTrue(all('力量輪廓' in option.label for option in crystal_select.options))
        ...
        preview = str(view.embed().to_dict())
        self.assertIn('替換預覽', preview)
        self.assertIn(f'#{first}', preview)
        self.assertIn(f'#{second}', preview)
        self.assertIn('力量輪廓', preview)
