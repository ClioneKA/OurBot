        ...
        while not fightDone:
            ...
            while not hostDone:
                try:
                    ...
                    break
                try:
                    responseHost = await self.bot.wait_for('message', check=message_check(channel=ctx.author.dm_channel), timeout=20.0)
            ...
            while not targetDone:
                try:
                    ...
                    break
                try:
                    responseTarget = await self.bot.wait_for('message', check=message_check(channel=target.dm_channel), timeout=20.0)
                except asyncio.TimeoutError:
            ...
            if responseHost.content == responseTarget.content:
                ...
                if responseHost.content > responseTarget.content:
                    ...
                    if responseHost.content == '1' and responseTarget.content == '3':
                        ...
                        result = f'{target.mention}勝！'
            await ctx.send(f'{ctx.message.author.mention}出{srp[responseHost.content]}！ {target.mention}出{srp[responseTarget.content]}！ {result}')
