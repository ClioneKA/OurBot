...
class EquipmentView(discord.ui.View):
    ...
    def rebuild(self):
        ...
        selected_item = self.available_items.get(self.item_id)
        can_socket = bool(selected_item and selected_item.socket_base)
        if can_socket:
        ...
        async with self.lock:
            ...
            try:
                if action == 'slot':
                    ...
                    changed = self.cog.characters.get_instance(self.guild_id, self.owner.id, self.item_id)
                    notice = f'已消耗 {ITEMS[PAINT_ITEMS[self.paint_color]].name}，裝備變為 {self.cog.characters.resolved_item(changed).name}。'
