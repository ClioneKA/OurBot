...
class ShopView(discord.ui.View):
    ...
    def rebuild(self):
        ...
        self.catalog.append('painting:balloon')
        ...
        self.add_item(PanelSelect('item', row=0, placeholder='選擇商品' if self.catalog else 'Lv.10 轉職後可購買職業裝備',
            disabled=not self.catalog, options=[discord.SelectOption(label=ITEMS[key].name, value=key,
                description=(f'{BALLOON_PAINTING_PROOF_COST} 討伐之證｜繪畫之影路線'
    ...
    def embed(self, notice=None):
        ...
        proofs = self.cog.characters.inventory_counts(self.guild_id, self.owner.id).get('proof:raid', 0)
        embed = discord.Embed(title='裝備商店',
        ...
        async with self.lock:
            ...
            try:
                if action == 'item':
                    ...
                    if self.item_id == 'painting:balloon':
