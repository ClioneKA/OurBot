...
def guide_embed(s, topic='intro'):
    ...
    if topic == 'intro':
        ...
        embed.description = (
            '**釣魚與農耕**\n'
            '生活頁可選擇時間開始釣魚，也能在四塊已解鎖農地種植；釣魚 Lv.80／100 可前往沉沒神殿潮池／星蝕外海，農耕 Lv.80 可為每塊田選擇豐收或研習專精。釣魚與農耕都可設定完成私訊。所有魚、作物、水草與藥草都能在酒館選取五份製成料理。\n\n'
            '**瑪格的占卜室**\n'
            '冒險頁可前往瑪格的占卜室；每天可不限次數支付逐次提高的金幣抽牌，為下一場討伐取得特殊效果與 10% 額外經驗。\n\n'
            '**冒險者酒館**\n'
            '每日委託於台灣時間 00:00 更新：安安徵求指定一般魔物的勝場，既有好感度 +1（不占聊天變動次數）；漢娜徵求食材，依食材品質 1～5，整張委託好感度 +1～5。每人每天各完成一次，好感度上限 100。無須接取，勝場依結算日期累計（含懸賞，不含管理員召喚），食材交付時扣除，可使用庫存；獎勵須當日回酒館領取。\n\n'
            '使用 /酒館 或從冒險頁前往冒險者酒館，製作並分享料理、公開請最多 5／10／20 人喝一杯，或支付金幣張貼不發金幣且不影響正常排程的額外懸賞。料理與請客會發布到酒館餐點頻道，日常聊天則使用獨立的冒險者聊天頻道；過期料理公告會自動刪除。\n\n'
            '**漢娜的裁縫所**\n'
            '漢娜的裁縫所染色原價 1,000 金幣，刺繡原價 500 金幣。多格飾品可分格刺繡，'
            '每件最多一個魔女刺繡。釣魚與農耕 Lv.60 後可取得綁定專屬材料，各消耗 10 份與 5,000 金幣製作飾品。'
            '漢娜好感度達 25／50／75／100 時可領取或升級胸針；每點好感度另減免 0.25% 加工費，最高 25%。\n\n')
    elif topic == 'advanced':
        embed.description = (
            '**聊天經驗倍率**\n'
