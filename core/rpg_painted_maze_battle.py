...
FINAL_MAX_ROUNDS = 40
PAINTING_TRAITS = {
    '巨獸': '每三回合橫掃全隊，可用護衛與全隊治療應對。',
    '毒蛛': '攻擊命中附帶中毒，可用淨化解除。',
    '月影妖狐': '每三回合施放單體月影斬並提高閃避，可用高命中攻擊應對。',
    '血翼蝠王': '每兩回合汲血撕咬，依實際扣血量回血；降低承傷也能減少回血。',
    '哥布林戰團': '隊長與兩名打手各有獨立血量；隊長每三回合鼓舞全團，可優先擊倒或暈眩隊長。',
    '荊棘妖樹': '每三回合回血並暈眩部分隊員，可用淨化解除暈眩。',
    '鐵殼魔像': '每三回合蓄力，下一回合施放重拳；可用盾擊打斷，破甲有助突破防禦。',
    '史萊姆群': '三隻史萊姆各有獨立血量；群攻能同時削血，逐隻擊倒可減少敵方行動。',
    '深淵鐘龍': '蓄力時獲得鐘甲，直接命中可削減層數；未破甲會釋放高傷全體攻擊，免疫暈眩。',
    '王城傀儡師': '劍傀儡護主、咒傀儡替本體減傷；每三回合修復，半血後吸收存活傀儡。',
    '瘟疫縫合獸': '攻擊疊加腐敗，三層在行動前爆裂；治療腐敗者會使怪物回血，可先淨化。',
    '赤雷與蒼炎': '赤雷奇數回合單體攻擊，蒼炎偶數回合全體攻擊；每四回合雷炎吐息，需在再生共鳴前擊倒另一隻。',
    '吞城鯨': '鯨脂提供減傷，可用高傷單體攻擊逐層擊破；水位持續上升，滿水位後蓄力吞城。',
    '熔爐鎧獸': '爐甲提供減傷；每四回合補甲並蓄力全體震爆，可用破甲或盾擊打斷。',
    '迷霧菌后': '菌后與兩隻孢子各有獨立血量；孢子每三回合膨脹，下一回合爆炸並中毒全隊，可擊倒或盾擊打斷。',
    '星蝕巨神': '星核保護巨神；巨神每四回合蓄力全體攻擊，擊倒星核可取消蓄力並使本體破甲。',
    '逆潮聖骸': '攻擊疊加可淨化的溺印；每三回合召喚法陣，擊倒法陣可阻止全體傷害與治療。',
}


def encounter_traits(room):
    if room['boss_index'] < len(room['paintings']):
        painting = room['paintings'][room['boss_index']]
        return PAINTING_TRAITS.get(painting['base_kind'], '依畫作的攻擊模式自動戰鬥。')
    text = ('依本場三份契約使用色彩技能，同色反噬疊加；HP 70% 時清除短期負面狀態，'
            '35% 時進入最後階段。各份契約效果見「尾王反噬」。')
    if room['route'] == 'noah':
        text += '70% HP 時召喚未乾色塊，需及時擊倒以免強化色彩技能。'
    return text
