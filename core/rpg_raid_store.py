...
DROP_TABLES = {
    ...
    '赤雷與蒼炎': ('twin_beast:infantry:weapon', 'twin_beast:infantry:suit',
                'twin_beast:archer:weapon', 'twin_beast:archer:suit', 'twin_beast:charm'),
    '吞城鯨': ('whale:knight:weapon', 'whale:knight:suit',
    ...
    def create(self, guild, channel, monster, now, reward_policy=None, reward_overrides=None,
        ...
        with self.db:
            ...
            if fishing_encounter is not None:
                ...
                if is_legacy_expedition_active(self.db, row[1], now):
                    raid['members'] = []
        ...
        with self.db:
            ...
            if raid['status'] in ('posting', 'lobby', 'running'):
                for user in raid['members']:
                    require_not_legacy_expedition(self.db, user)
            self._save(raid)
    ...
    def join(self, raid_id, guild, user, now, maximum, leave=False):
        with self.db:
            ...
            if leave:
                ...
                require_not_legacy_expedition(self.db, user, now)
