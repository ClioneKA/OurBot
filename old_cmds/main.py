...
goose_emoji = [
    "<:goose_suck:1021737115412860988>",
    "<:goose_suck2:1021737380312535130>",
    "<a:goosepat:847511782121799730>",
...
class Main(Cog_Extension):
    ...
    async def adol(self, ctx):
        gres = requests.get(
            "https://apps.runescape.com/runemetrics/profile/profile?user=nijiiro_nino&activities=20"
        )
        gdata = gres.json()
        for skill in gdata["skillvalues"]:
            if skill["id"] == 16:
                await ctx.channel.send(
                    ...
                    + skill["level"].__str__()
                    + "等，距離敏捷大師還差"
                    + ((2000000000 - skill["xp"]) / 10).__str__()
                    + "經驗。"
                )
